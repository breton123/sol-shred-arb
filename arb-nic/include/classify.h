#ifndef ARB_NIC_CLASSIFY_H
#define ARB_NIC_CLASSIFY_H

#include <stdint.h>
#include <string.h>

/*
 * v1 relevance: exact 32-byte program-id scan of the shred payload.
 * No SIMD. No decode. No maps. Two IDs only.
 *
...
/* Scan payload windows of 32 bytes. Returns REL_* bitmask. */
static inline uint32_t
classify_relevance(const uint8_t *payload, uint16_t len)
{
    ...
    for (i = 0; i <= last; i++) {
        if ((hit & REL_DLMM) == 0 && id_eq32(payload + i, PROG_DLMM)) {
            hit |= REL_DLMM;
        }
        if ((hit & REL_PUMP) == 0 && id_eq32(payload + i, PROG_PUMP)) {
            hit |= REL_PUMP;
        }
        if (hit == REL_BOTH) {
            break;
