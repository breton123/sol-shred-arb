#!/bin/bash
# Restart paper_orbit only — add opp_synced audit. Leave feed / STATE-005 / expand_loop.
set -e
python3 /home/louis/arb-cap/state005/crlf.py \
  ...
  /home/louis/arb-cap/state005/watch_first_opp.py
set -a
...
echo "state006 pid $!"
nohup python3 /home/louis/arb-cap/state005/watch_first_opp.py \
  >> /home/louis/captures/paper_orbit/watch_first_opp.log 2>&1 &
echo "watch_first pid $!"
sleep 1
pgrep -c feed_live
pgrep -af "paper_orbit|state006.py|watch_first_opp|expand_loop|state005_n|capture.py" || true
